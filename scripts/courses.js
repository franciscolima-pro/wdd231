...
const completedCourses = [110, 130, 111, 210, 131]
for (let i = 0; i < courses.length; i++){
    if(completedCourses.includes(courses[i].number)){
...
function calculateCredits(coursesArray) {
    return coursesArray.reduce((total, course) => {
        return total + (course.completed ? course.credits : 0); // Conta créditos apenas dos cursos concluídos
    }, 0);
}

// Exibe os créditos totais
